...

///////////////////////////////////////////// BETWEEN THESE BLOCKS ARE THE FUNCTIONS WE NEED 

WINDOW::WINDOW(int x,int y,int width,int height,char *title): X ( x),  Y (y), WIDTH (width),HEIGHT (height),  TITLE(title)
{
      
        if(SDL_Init(SDL_INIT_VIDEO) !=0)
		    Print("Init Video Error");

//=======STEP 1: ===================================================================================================================================-=====      
//____________________________________________ Create our Window and Render_______________________________________________________________________________
 
		HWND = SDL_CreateWindow(TITLE, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, WIDTH, HEIGHT, 0); 
		if(!HWND)
			Print("ERROR: Initializing SDL Window");

        RENDER = SDL_CreateRenderer(HWND, -1, SDL_RENDERER_ACCELERATED);  
		if(!RENDER)
			Print("ERROR: Initializing SDL Renderer");
...
//===== STEP 2: ==========================================================================================================================================     
//___________________ Create a Texture that our Render can use & set it to Streaming so it keeps the Channle open contantly ______________________________
...
//      ^ ^ ^ ^ ^ ^ ^ ^ ^ ^ <-- Set the Texture we just made to be out Render Target
        ...
        WINDOW_PIXELS = new Uint32[WIDTH * (HEIGHT + 400) + 1];   // Create a pixel buffer that we will send all at once to the GPU 
        memset(WINDOW_PIXELS, 0, WIDTH * HEIGHT * sizeof(Uint32));// Clears the buffer to a single color, Black in this case

//=======um... STEP 3: ===========ENABLE IMAGE LOADING FOR WINDOW=========================================================================================
//       LOL, Leave vauge comments about image loading that make no sense a month later :D
}

void SET_PIXEL(int x, int y, Uint32 COLOR)
{
	SCREEN->WINDOW_PIXELS[(y * SCREEN->WIDTH) + x] = COLOR; // WRITE DIRECTLY TO PIXEL BUFFER;
}


void CLS(){
//------STEP 4:--------- Update out backbuffer with the pixel data we just wrote to ----------------------------------------------------------------------
				SDL_UpdateTexture(SCREEN->BACK_BUFFER, NULL, SCREEN->WINDOW_PIXELS, SCREEN->WIDTH * sizeof(Uint32)); 

//------STEP 5:-------------CLEAR OUR PIXEL DATA ---------------------------------------------------------------------------------------------------------
...
//------STEP 6:------------- Submit out DRAW CALL to the GPU and HAVE THE RENDER DO ITS JOB --------------------------------------------------------------
        SDL_RenderCopy(SCREEN->RENDER, SCREEN->BACK_BUFFER, NULL, NULL);
//-----STEP 7:----------------DISPLAY OUR RENDER----------------------------------------------------------------------------------------------------------
...
//====================== CALCULATE FPS ===================================================================================================================
	    ...
	    if(SDL_GetTicks() - SCREEN->TIMER > 1000){
		    SCREEN->FPS =  SCREEN->FRAME_COUNTER;// / ((SDL_GetTicks() - SCREEN->TIMER)/1000) ;
			SCREEN->TIMER = SDL_GetTicks();
				   SCREEN->FRAME_COUNTER=0;
		}

		//if(SCREEN->FPS > 60){SDL_Delay(SCREEN->FPS/10);}
...
// POSSIBLE IMPROVEMENT COULD BE HAD BUT I HAVE NOT FIGURED OUT HOW TO USE IT YET. OTHER LOCKING METHODS Showed 10-20x increase in performance
// If that holds true here then we will see FPS close to the thosands for large screens
void LOCK_PIXELS(){
	int   PITCH;
	void *PIXELS;
...
//notes and unfinished shit
/*bool TEXTURE_COPY(SDL_Texture *srctexture, SDL_Texture *destTexture, SDL_Rect rSource,SDL_Rect rDest)
{
	Print ("YOU HAVE NOT FINISHED TEXTURE COPY YET");

	int height = rSource.h,
		 width = rSource.w;
        for(int Y = rSource.y; Y < (rSource.y + rSource.h); Y++)
		{
	            for(int X = rSource.x; X < (rSource.x +  rSource.w); X++)
				{
				//	memcpy(srctexture[X + (*srctexture.) * Y], destTexture[X], );
				}
		}
}*/
